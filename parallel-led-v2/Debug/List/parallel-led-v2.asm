...
;#define THRESHOLD_B 170
;#define THRESHOLD_C 290
;#define THRESHOLD_D 430
;
;
;unsigned char a=0x01,b=0x01,c=0x01,d=0x01;
;unsigned long timeA=0,timeB=0,timeC=0,timeD=0;
;
;// Global flag used to signal that 10ms have elapsed
...
; .FSTART _timer1_compa_isr
	ST   -Y,R30
; 0000 0013     timer1_flag = 1;  // Set flag when compare match occurs (10ms elapsed)
	LDI  R30,LOW(1)
	STS  _timer1_flag,R30
; 0000 0014 }
	LD   R30,Y+
	RETI
; .FEND
;
;// Function to create a ~10ms delay using Timer1 and sleep (Idle mode)
;void sleep_timer1_ms(int x)
; 0000 0018 {
_sleep_timer1_ms:
; .FSTART _sleep_timer1_ms
; 0000 0019     timer1_flag = 0;  // Clear the flag
	ST   -Y,R27
	ST   -Y,R26
;	x -> Y+0
	LDI  R30,LOW(0)
	STS  _timer1_flag,R30
; 0000 001A 
; 0000 001B     // --- Configure Timer1 in CTC Mode ---
; 0000 001C     // WGM12: CTC mode, and CS11: prescaler of 8.
; 0000 001D     // With an 8MHz clock and prescaler 8, Timer1 ticks at 1MHz (1µs per tick).
; 0000 001E     // For a 10ms delay, we need 10,000 ticks (0 to 9999).
; 0000 001F     TCCR1A = 0x00;                        // Normal port operation
	OUT  0x2F,R30
; 0000 0020     TCCR1B = (1 << WGM12) | (1 << CS11);    // CTC mode with prescaler = 8
	LDI  R30,LOW(10)
	OUT  0x2E,R30
; 0000 0021     TCNT1  = 0;                           // Reset Timer1 counter
	LDI  R30,LOW(0)
	LDI  R31,HIGH(0)
	OUT  0x2C+1,R31
	OUT  0x2C,R30
; 0000 0022     OCR1A  = 999 * x;                        // Set compare value to 9999 for 10ms
	LD   R30,Y
	LDD  R31,Y+1
	LDI  R26,LOW(999)
	LDI  R27,HIGH(999)
	...
	OUT  0x2A,R30
; 0000 0023 
; 0000 0024     // Enable Timer1 Compare Match A interrupt
; 0000 0025     TIMSK |= (1 << OCIE1A);
	IN   R30,0x39
	ORI  R30,0x10
	OUT  0x39,R30
; 0000 0026 
; 0000 0027     // --- Set Up Idle Sleep Mode Manually ---
; 0000 0028     // For Idle mode on the ATmega32, the sleep mode bits (SM0 and SM1 in MCUCR)
; 0000 0029     // should be 0. Then, set the Sleep Enable (SE) bit to allow sleeping.
; 0000 002A     MCUCR &= ~((1 << SM0) | (1 << SM1));  // Select Idle mode (SM1=0, SM0=0)
	IN   R30,0x35
	ANDI R30,LOW(0xCF)
	OUT  0x35,R30
; 0000 002B     MCUCR |= (1 << SE);                   // Enable sleep mode
	IN   R30,0x35
	ORI  R30,0x80
	OUT  0x35,R30
; 0000 002C 
; 0000 002D     // Wait until Timer1 Compare Match A interrupt sets the flag.
; 0000 002E     while(!timer1_flag)
_0x4:
	LDS  R30,_timer1_flag
	CPI  R30,0
	BRNE _0x6
; 0000 002F     {
; 0000 0030         // Execute the sleep instruction. In Idle mode, Timer1 keeps running.
; 0000 0031         #asm("sleep");
	sleep
; 0000 0032     }
	RJMP _0x4
_0x6:
; 0000 0033 
; 0000 0034     // After waking, disable sleep.
; 0000 0035     MCUCR &= ~(1 << SE);
	IN   R30,0x35
	ANDI R30,0x7F
	OUT  0x35,R30
; 0000 0036 
; 0000 0037     // --- Stop Timer1 ---
; 0000 0038     TIMSK &= ~(1 << OCIE1A);  // Disable Timer1 Compare Match A interrupt
	IN   R30,0x39
	ANDI R30,0xEF
	OUT  0x39,R30
; 0000 0039     TCCR1B = 0;              // Stop Timer1 by clearing its clock source
	LDI  R30,LOW(0)
	OUT  0x2E,R30
; 0000 003A }
	ADIW R28,2
...
; 0000 0049 DDRB=(1<<DDB7) | (1<<DDB6) | (1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0);
	LDI  R30,LOW(255)
	OUT  0x17,R30
...
; 0000 00A7       {
; 0000 00A8 
; 0000 00A9       if (timeA <= timeB && timeA <= timeC && timeA <= timeD) {
	RCALL SUBOPT_0x0
	RCALL SUBOPT_0x1
	BRLO _0xB
	RCALL SUBOPT_0x2
...
; 0000 00AA         PORTA = a;
	OUT  0x1B,R5
; 0000 00AB         a = (a << 1) | (a >> 7);     // 1 left shift and and check for overflow
	MOV  R30,R5
	LSL  R30
	MOV  R26,R30
	MOV  R30,R5
	RCALL SUBOPT_0x4
	MOV  R5,R30
; 0000 00AC         sleep_timer1_ms(timeA);
	LDS  R26,_timeA
	LDS  R27,_timeA+1
	RCALL _sleep_timer1_ms
; 0000 00AD         timeB-=timeA;
	RCALL SUBOPT_0x5
	RCALL SUBOPT_0x6
; 0000 00AE         timeC-=timeA;
	RCALL SUBOPT_0x5
	RCALL SUBOPT_0x7
; 0000 00AF         timeD-=timeA;
	RCALL SUBOPT_0x5
	RCALL SUBOPT_0x8
; 0000 00B0         timeA = THRESHOLD_A;
	__GETD1N 0x3C
	RCALL SUBOPT_0x9
; 0000 00B1         }
; 0000 00B2 
; 0000 00B3     else if (timeB <= timeA && timeB <= timeC && timeB <= timeD) {
	RJMP _0xD
_0xA:
	RCALL SUBOPT_0xA
	RCALL SUBOPT_0xB
...
; 0000 00B5         b = (b << 1) | (b >> 7);
	...
	RCALL SUBOPT_0x4
	MOV  R4,R30
; 0000 00B6         sleep_timer1_ms(timeB);
	LDS  R26,_timeB
	LDS  R27,_timeB+1
	RCALL _sleep_timer1_ms
; 0000 00B7         timeA-=timeB;
	RCALL SUBOPT_0xC
	RCALL SUBOPT_0xD
; 0000 00B8         timeC-=timeB;
	RCALL SUBOPT_0xC
	RCALL SUBOPT_0x7
; 0000 00B9         timeD-=timeB;
	RCALL SUBOPT_0xC
	RCALL SUBOPT_0x8
; 0000 00BA         timeB = THRESHOLD_B;
	__GETD1N 0xAA
	STS  _timeB,R30
	STS  _timeB+1,R31
	STS  _timeB+2,R22
...
; 0000 00BB         }
; 0000 00BC 
; 0000 00BD     else if (timeC <= timeA && timeC <= timeB && timeC <= timeD) {
	RJMP _0x11
_0xE:
	RCALL SUBOPT_0xA
	RCALL SUBOPT_0xE
...
; 0000 00BF         c = (c << 1) | (c >> 7);
	...
	RCALL SUBOPT_0x4
	MOV  R7,R30
; 0000 00C0         sleep_timer1_ms(timeC);
	LDS  R26,_timeC
	LDS  R27,_timeC+1
	RCALL _sleep_timer1_ms
; 0000 00C1         timeA-=timeC;
	RCALL SUBOPT_0xF
	RCALL SUBOPT_0xD
; 0000 00C2         timeB-=timeC;
	RCALL SUBOPT_0xF
	RCALL SUBOPT_0x6
; 0000 00C3         timeD-=timeC;
	RCALL SUBOPT_0xF
	RCALL SUBOPT_0x8
; 0000 00C4         timeC = THRESHOLD_C;
	__GETD1N 0x122
	STS  _timeC,R30
	STS  _timeC+1,R31
	STS  _timeC+2,R22
...
; 0000 00C9         d = (d << 1) | (d >> 7);
	...
	RCALL SUBOPT_0x4
	MOV  R6,R30
; 0000 00CA         sleep_timer1_ms(timeD);
	LDS  R26,_timeD
	LDS  R27,_timeD+1
	RCALL _sleep_timer1_ms
; 0000 00CB         timeA-=timeD;
	RCALL SUBOPT_0x10
	RCALL SUBOPT_0xD
; 0000 00CC         timeB-=timeD;
	RCALL SUBOPT_0x10
	RCALL SUBOPT_0x6
; 0000 00CD         timeC-=timeD;
	RCALL SUBOPT_0x10
	RCALL SUBOPT_0x7
; 0000 00CE         timeD = THRESHOLD_D;
	__GETD1N 0x1AE
	STS  _timeD,R30
	STS  _timeD+1,R31
	STS  _timeD+2,R22
